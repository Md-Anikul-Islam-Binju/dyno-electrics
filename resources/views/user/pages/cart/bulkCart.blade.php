...
@section('content')
        ...
        <div class="container">
            <div class="row">
                <div class="content theme_ws_box p_15">
                    <h1 class="title">Bulk Shopping Cart</h1>
                    @php
                        $subTotal = 0; // Initialize subtotal variable
                    @endphp
                    @if(count($bulkCart) > 0)
                        <form id="cartForm" action="{{route('bulk.cart.update')}}" method="POST">
                            @csrf
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered cart-table bg-white">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-left text-center">Description on Size Info</td>
                                    ...
                                    @foreach($bulkCart as $key => $item)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end table_data_none">
                                                <textarea placeholder="As Like Define Your size(M=10,XXL=5)" name="size_info[{{ $key }}]" rows="5">{{ $item['size_info'] ?? '' }}</textarea>
                                                <button type="button" class="btn btn-primary btn-sm mt-2 update-size-info" data-product-id="{{ $key }}">Update</button>
                                            </td>
                                            <td class="text-end table_data_none">
                                                {{ $item['price'] }} ৳
                                            </td>
                                            ...
                                            <td class="text-end">
                                                <a href="{{ route('bulk.cart.item.remove', $key) }}" class="btn btn-danger btn-sm">
                        ...
                        <div class="buttons">
                            <div class="pull-right">
                                <a href="{{route('frontend.bulk.product')}}" class="btn_style">Continue Shopping</a>
                            </div>
                            ...
                            @if($user)
                                <div class="pull-right">
                                    <a href="{{ route('checkout.bulk.order') }}" class="btn_style">Checkout</a>
                                </div>
                            @else
                                <div class="pull-right">
                                    <a href="{{route('checkout.as.guest.order.bulk')}}" class="btn_style" style="background-color: red">Checkout as Guest</a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Handle size info update
            form.querySelectorAll('.update-size-info').forEach(button => {
                button.addEventListener('click', function() {
                    const productId = this.getAttribute('data-product-id');
                    actionInput.value = 'update_size_info';
                    productIdInput.value = productId;
                    form.submit();
                });
            });
